#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║  TrendR — Trend Research Installer (OpenClaw runtime)           ║
...
### /tr 交互模式（参数化入口）
主入口改为 \`/tr\`（兼容旧入口 \`/trendr\`）。

当用户输入 \`/tr\`、\`/tr 主题...\`、\`/trendr\`、\`trendr 研究 ...\`（或任意包含 \`trendr\` 的研究请求）时，先收集参数，再执行：
- 研究主题（必填，一句话）
...
echo -e "  ${DIM}卸载: bash uninstall.sh --openclaw${NC}"
echo ""
