...
### /trendr 交互模式（参数化入口）
当用户输入 \`/trendr\`、\`/trendr 主题...\`、\`trendr 研究 ...\`（或任意包含 \`trendr\` 的研究请求）时，默认先进入快速确认模式（\`y/n/r\`）：

- 若仅给了主题，也先进入快速确认，不得直接开跑
