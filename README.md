...
- 主入口升级：`/trendr` 简化为 `/tr`（保留 `/trendr` 兼容）。
- 修复 TrendR 入口一致性：`/tr`、`/tr 主题...`、`/trendr`、`trendr 研究 ...` 统一进入参数化交互，不再直接开跑。
- 恢复参数化交互：默认快速模式 + `/b` 精确模式（A/B/C + 时间预算）。
...
输入 `/tr`、`/tr 主题...`、`/trendr` 或 `trendr 研究 ...` 后，默认进入参数化快速模式；若要进入精确模式，输入 `/b`。
...
精确模式（`/b`）：
- 在上述参数基础上进行更细粒度调整（仍按 A/B/C + 时间预算）
